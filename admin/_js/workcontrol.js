...
$(
    function () {
        ...
        $('html').on('change', 'form.auto_save', function (e) {
            ...
            form.ajaxSubmit({
                ...
                success: function (data) {
                    ...
                    //EXIBE CALLBACKS
                    //cities return for UF
                    if (data.city) {
                        let CallBackCities = $('.cities_return');
                        if (CallBackCities.length) {
                            CallBackCities.html(data.city);
                            $('.cities_return').fadeIn('slow');
                        } else {
                            Trigger(data.city);
                        }
                    }
                    
                    //all_cities return for UF
                    if (data.allcities) {
                        let CallBackAllCities = $('.allcities');
                        if (CallBackAllCities.length) {
                            CallBackAllCities.empty().append(data.allcities).fadeIn('slow');
                        }
                    }
                    
                    if (data.placeholder) {
                        let CallBackPresent = $('.allcities');
                        CallBackPresent.attr('placeholder', data.placeholder).empty();
                    }
            ...
            form.ajaxSubmit({
                ...
                success: function (data) {
                    ...
                    form.find('.form_load').fadeOut('slow', function () {
                        ...
                        //INTERAGE COM TINYMCE VIDEO
                        if (data.tinyMCEvideo) {
                            tinyMCE.activeEditor.insertContent(data.tinyMCEvideo);
                            $('.workcontrol_videoupload').fadeOut('slow');
                        }
                        ...
                        if (data.field) {
                            form.find('[name="' + data.field + '"]').addClass('form_error');
                        } else {
                            $('input, select, textarea').removeClass('form_error');
                        }
        ...
        $('.wc_loadimage').change(function () {
            ...
            if (this.files && (this.files[0].type.match("image/jpeg") ||
                this.files[0].type.match("image/png") ||
                this.files[0].type.match("image/gif") ||
                this.files[0].type.match("image/webp"))
            ) {
                TriggerClose();
                ...
                Trigger('<div class="trigger trigger_alert trigger_ajax"><b>ERRO AO SELECIONAR:</b>' +
                    ' O arquivo <b>' + this.files[0].name + '</b> não é válido! <b>Selecione uma imagem (.jpg, .jpeg,' +
                    ' .png, .gif ou .webp)</b></div>');
                target.fadeOut('fast', function () {
                    $(this).attr('src', fileDefault).fadeIn('slow');
                });
                input.val('');
        ...
        $('.wc_loadvideo').change(function () {
            var input = $(this);
            var target = $('.video_preview'); // Alvo padrão para visualização
            var fileDefault = target.find('source').attr('src') || '';
            
            if (!input.val()) {
                target.fadeOut('fast', function () {
                    target.find('source').attr('src', fileDefault);
                    target[0].load();
                    target.fadeIn('slow');
                });
                return false;
            ...
            if (this.files && (this.files[0].type.match("video/mp4") ||
                this.files[0].type.match("video/webm"))
            ) {
                TriggerClose();
                ...
                reader.onload = function (e) {
                    target.fadeOut('fast', function () {
                        target.find('source').attr('src', e.target.result);
                        target[0].load(); // recarrega o vídeo com o novo source
                        target.fadeIn('fast');
                    });
                };
                reader.readAsDataURL(this.files[0]);
                ...
                Trigger('<div class="trigger trigger_alert trigger_ajax"><b>ERRO AO SELECIONAR:</b>' +
                    ' O arquivo <b>' + this.files[0].name + '</b> não é válido! <b>Selecione um vídeo (.mp4 ou .webm)</b></div>');
        ...
        $(document).on("ready click", function () {
            ...
            if ($("input[name=show_headline]").is(":checked")) {
                $("input[name=slide_headline]").removeAttr('disabled');
                $("input[name=slide_headline]").parent().fadeIn();
            } else {
                $("input[name=slide_headline]").prop('disabled', true);
                $("input[name=slide_headline]").parent().fadeOut();
            }
            if ($("input[name=show_desc]").is(":checked")) {
                $("textarea[name=slide_desc]").removeAttr('disabled');
                $("textarea[name=slide_desc]").parent().fadeIn();
            } else {
                $("textarea[name=slide_desc]").prop('disabled', true);
                $("textarea[name=slide_desc]").parent().fadeOut();
            }
            
            //LINKS
            if ($("input[name=slide_product]").is(":checked")) {
                $("input[name=slide_link_pdt], input[name=slide_link_pdt_btn]").removeAttr('disabled');
                $("input[name=slide_link_pdt], input[name=slide_link_pdt_btn] ").parent().fadeIn();
            } else {
                $("input[name=slide_link_pdt], input[name=slide_link_pdt_btn]").prop('disabled', true);
                $("input[name=slide_link_pdt], input[name=slide_link_pdt_btn] ").parent().fadeOut();
            }
            
            if ($("input[name=slide_category]").is(":checked")) {
                $("input[name=slide_link_cat], input[name=slide_link_cat_btn]").removeAttr('disabled');
                $("input[name=slide_link_cat], input[name=slide_link_cat_btn] ").parent().fadeIn();
            } else {
                $("input[name=slide_link_cat], input[name=slide_link_cat_btn]").prop('disabled', true);
                $("input[name=slide_link_cat], input[name=slide_link_cat_btn] ").parent().fadeOut();
            }
