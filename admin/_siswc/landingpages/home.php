...
if (DB_AUTO_TRASH !== 0) {
    ...
    // AUTO TRASH IMAGES
    $Read->fullRead(
        'SELECT page_cover, page_mockup FROM ' . DB_LANDING_PAGES . ' WHERE page_title IS NULL AND page_status = :st',
        'st=0'
    );
...
<header class="dashboard_header">
	...
	<div class="dashboard_header_search">
		<a title="Nova Página de Cliente" href="dashboard.php?wc=landingpages/create" class="btn btn_green icon-plus">Adicionar
			Nova Landing Page</a>
	</div>
...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()) {
        ...
        echo Check::erro(
            'Ainda não existem páginas cadastradas. Comece agora mesmo criando a primeira.',
            E_USER_NOTICE
        );
    } else {
        foreach ($Read->getResult() as $PAGE) {
            extract($PAGE);
            $page_status = (1 == $page_status ? '<span class="icon-checkmark font_green">Publicada</span>' : '<span class="icon-newspaper font_vintage">Rascunho</span>');
            $page_cover = (empty($page_cover) ? '' : BASE . sprintf(
                    '/tim.php?src=uploads/landingpages/%s&w=',
                    $page_social_media
            ...
            echo "<article class='box box25 page_single' id='{$page_id}'>
                ...
                <div class='box_content wc_normalize_height'>
                    <h1 class='title'>
                    <a title='Ver página no site' target='_blank' href='" . BASE . "/{$page_name}'>{$page_title}</a>
                    </h1>
                    <p>{$page_status}</p>
                </div>
