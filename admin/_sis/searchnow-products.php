...
<header class="dashboard_header">
	<div class="dashboard_header_title">
		<h1 class="icon-search">Relatório de Pesquisas por Produtos</h1>
...
$Page = ($getPage ?? 1);
$Pager = new Pager('dashboard.php?wc=searchnow-products&page=', '<<', '>>', 5);

$Pager->exePager($Page, 24);
$Read->exeRead(
    DB_SEARCH,
    sprintf(
        ' WHERE 1 = 1 %s AND search_publish IS NULL AND search_origin = :pdt ORDER BY search_commit DESC, search_count DESC LIMIT :limit OFFSET :offset',
        $Where
    ),
    sprintf('pdt=PROD&limit=%d&offset=%d&%s', $Pager->getLimit(), $Pager->getOffset(), $ParseString)
);
if (!$Read->getResult()) {
    $Pager->returnPage();
    echo Check::erro(
        '<span>Seus usuários ainda não pesquisaram por produtos no site. Assim que isso acontecer você poderá receber dicas de conteúdo pelas pesquisas realizadas!</span>',
        E_USER_NOTICE
    );
} else {
...
?>
    ...
    $Pager->exePaginator(
        DB_SEARCH,
        sprintf(' WHERE 1 = 1 %s AND search_publish IS NULL AND search_origin = :pdt ', $Where),
        'pdt=PROD&'.$ParseString
    );
