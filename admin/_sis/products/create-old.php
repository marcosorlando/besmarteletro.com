                                        ...
                                        if ($Read->getResult()):
                                            foreach ($Read->getResult() as $SIZE):
                                                ...
                                                if ($Read->getResult()):
                                                    ...
                                                    echo "<label><span style='{$bgPrint}' title='{$PRINT['attr_print_title']}'><input type='hidden' name='print_id[]' value='{$PRINT['attr_print_id']}'/></span><span title='{$SIZE['attr_size_title']}'>{$SIZE['attr_size_code']}<input type='hidden' name='size_id[]' value='{$SIZE['attr_size_id']}'/></span><input name='stock_inventory[]' type='number' min='0' value='0'>{$PRINT['attr_print_title']}</label>";
                                                endif;
                                            endforeach;
                                        endif;
                            ...
                            <?php
                                ...
                                if ($Read->getResult()):
                                    foreach ($Read->getResult() as $COLOR):
                                        ...
                                        if ($Read->getResult()):
                                            echo "<label><span style='{$bgColor}' title='{$COLOR['attr_color_title']}'><input type='hidden' name='color_id[]' value='{$COLOR['attr_color_id']}'/></span><p title='default'>{$COLOR['attr_color_title']}</p><input name='stock_inventory[]' type='number' min='0' value='{$Read->getResult()[0]['stock_inventory']}'></label>";
                                        else:
                                            echo "<label><span style='{$bgColor}' title='{$COLOR['attr_color_title']}'><input type='hidden' name='color_id[]' value='{$COLOR['attr_color_id']}'/></span><p title='default'>{$COLOR['attr_color_title']}</p><input name='stock_inventory[]' type='number' min='0' value='0'></label>";
                                        endif;
                            ...
                            <?php
                                ...
                                if ($Read->getResult()):
                                    foreach ($Read->getResult() as $PRINT):
                                        ...
                                        if ($Read->getResult()):
                                            echo "<label><span style='{$bgPrint}' title='{$PRINT['attr_print_title']}'><input type='hidden' name='print_id[]' value='{$PRINT['attr_print_id']}'/></span><p title='default'>{$PRINT['attr_print_title']}</p><input name='stock_inventory[]' type='number' min='0' value='{$Read->getResult()[0]['stock_inventory']}'></label>";
                                        else:
                                            echo "<label><span style='{$bgPrint}' title='{$PRINT['attr_print_title']}'><input type='hidden' name='print_id[]' value='{$PRINT['attr_print_id']}'/></span><p title='default'>{$PRINT['attr_print_title']}</p><input name='stock_inventory[]' type='number' min='0' value='0'></label>";
                                        endif;
    ...
    <form class="auto_save" name="manage_pdt" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="box box70">
            <div class="box_content">
                ...
                <div class="label_50">
                    <label class="label">
                        <span class="legend">Capa (JPG <?= THUMB_W; ?>x<?= THUMB_H; ?>px) - máx: 1mb</span>
                        <input type="file" class="wc_loadimage" name="pdt_cover"/>
                    </label>

                    <label class="label">
                        <span class="legend">
                            <a href="<?= $pdtPDF ?>" target="_blank" title="Visualizar"> <i class="icon-file-pdf <?= !empty($pdt_drawing) && !is_dir($pdtPDF) ? 'font_green' : 'font_red' ?>"></i></a>Desenho Técnico (PDF - máx. 5mb): </span>
                        <input type="file" class="wc_load_pdf" name="pdt_drawing" value="<?= ($pdt_drawing ? $pdt_drawing : ''); ?>"/>
                    </label>
                </div>
                ...
                <div class="label_33">
                    <label class="label">
                        ...
                        <input type="text" name="pdt_code" value="<?= ($pdt_code ? $pdt_code : str_pad($pdt_id, 4, 0, STR_PAD_LEFT)); ?>" required/>
                    </label>
                ...
                <div class="label_33">
                    ...
                    <label class="label">
                        ...
                        <input type="text" name="pdt_unity" value="<?= $pdt_unity; ?>" placeholder="Unidade de Medida:" required/>
                    </label>

                    <label class="label">
                        <span class="legend">Preço de Venda em R$:</span>
                        <input type="text" name="pdt_price" value="<?= $pdt_price ? number_format($pdt_price, '2', ',', '.') : "0,00"; ?>"  class="formMoeda" placeholder="R$" required/>
                    </label>
                ...
                <div class="label_50">
                    <label class="label">
                        ...
                        <input type="number" step="0.01"  name="pdt_dimension_height" value="<?= $pdt_dimension_height; ?>" placeholder="Altura em Centímetros:" required/>
                    </label>

                    <label class="label">
                        <span class="legend">Largura Em Centímetros:</span>
                        <input type="number" step="0.01"  name="pdt_dimension_width" value="<?= $pdt_dimension_width; ?>" placeholder="Largura em Centímetros:" required/>
                    </label>
                    <div class="clear"></div>
                </div>
                ...
                <div class="label_50">
                    <label class="label">
                        <span class="legend">Espessura Em Milímetros:</span>
                        <input type="number" step="0.01"  name="pdt_dimension_depth" value="<?= $pdt_dimension_depth; ?>" placeholder="Espessura em Milimetros:" required/>
                    </label>

                    <label class="label">
                        <span class="legend">Peso Em Kilos:</span>
                        <input type="number" step="0.01" name="pdt_dimension_weight" value="<?= $pdt_dimension_weight; ?>" placeholder="KG:" required/>
                    </label>
                    <div class="clear"></div>
                </div>
        ...
        <div class="box box30">
            ...
            <div class="box_content">
                ...
                <label class="label">
                    ...
                    <input type="text" name="pdt_offer_price"  class="formMoney"  value="<?= $pdt_offer_price ? number_format($pdt_offer_price, '2', ',', '.') : "0,00"; ?>" placeholder="Preço Promocional:"/>
                </label>

                <label class="label">
                    <span class="legend">Início da Promoção:</span>
                    <input type="text" class="formTime jwc_datepicker" data-timepicker="true" name="pdt_offer_start" value="<?= ($pdt_offer_start ? date('d/m/Y H:i', strtotime($pdt_offer_start)) : null); ?>"/>
                </label>

                <label class="label">
                    <span class="legend">Fim da Promoção:</span>
                    <input type="text" class="formTime jwc_datepicker" data-timepicker="true"  name="pdt_offer_end" value="<?= ($pdt_offer_end ? date('d/m/Y H:i', strtotime($pdt_offer_end)) : null); ?>"/>
                </label>


                <p class="section">Infos Extras:</p>
                <!----------------------------------
                   ######## CUSTOM BY ALISSON #########
                   ----------------------------------->
                <label class="label">
                    <span class="legend">Cores e Estoque:</span>
                    <span class="wc_pdt_stock btn btn_blue"><span class="j_pdt_inventory"><?= $pdt_inventory; ?></span></span>
                </label>
