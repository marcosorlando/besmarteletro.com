            ...
            <div class="panel">
                ...
                if (!$Read->getResult()) {
                    ...
                    foreach ($Read->getResult() as $Comm) {
                        ...
                        $Photo = ($Read->getResult()[0]['user_thumb'] && file_exists(
                            '../uploads/' . $Read->getResult()[0]['user_thumb']
                        ) ? sprintf(
                            '../tim.php?src=uploads/%s&w=100&h=100',
                            $Read->getResult()[0]['user_thumb']
            ...
            <div id="panel_search" class="panel dashboard_search">
                ...
                if (!$Read->getResult()) {
                    ...
                    foreach ($Read->getResult() as $Search) {
                        extract($Search);
                        $Read->fullRead(
                            'SELECT pdt_id FROM ' . DB_PDT . " WHERE pdt_status = 1 AND (pdt_title LIKE '%' :s '%' OR pdt_subtitle LIKE '%' :s '%' OR pdt_tags LIKE '%' :s '%')",
                            's=' . $search_key
                        );
