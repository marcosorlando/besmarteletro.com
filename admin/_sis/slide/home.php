...
<div class="dashboard_content">
    <?php
        ...
        if (!$Read->getResult()):
            $Pager->ReturnPage();
            Erro("<span class='al_center icon-notification'>Ainda não existem conteúdos em destaque cadastrados em seu site. Comece cadastrando o primeiro!</span>", E_USER_NOTICE);
        else:
                ...
                <div class="wc_actions" style="background:#2E4051">
                    <div class="box box50" style="tex-align: left">
                        <label class="label_check label_publish <?= ($show_title == 1 ? 'active' : ''); ?>">
                            <input style="margin-top: -1px;" type="checkbox" value="1" name="show_title" <?= ($show_title == 1 ? 'checked' : ''); ?>> TEXTO DA CHAMADA
                        </label>
                        <label class="label_check label_publish <?= ($show_desc == 1 ? 'active' : ''); ?>">
                            <input style="margin-top: -1px;" type="checkbox" value="1" name="show_desc" <?= ($show_desc == 1 ? 'checked' : ''); ?>> TEXTO COMPLEMENTAR
                        </label>

                        <label class="label_check label_publish <?= ($slide_product == 1 ? 'active' : ''); ?>"><input style="margin-top: -1px;" type="checkbox" value="1" name="slide_product" <?= ($slide_product == 1 ? 'checked' : ''); ?>> PRODUTO
                        </label>
                        <label class="label_check label_publish <?= ($slide_category == 1 ? 'active' : ''); ?>"><input style="margin-top: -1px;" type="checkbox" value="1" name="slide_category" <?= ($slide_category == 1 ? 'checked' : ''); ?>> CATEGORIA</label>
                    </div>
                </div>
