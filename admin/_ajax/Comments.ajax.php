...
// Email é uma classe global carregada via bootstrap
...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack) {
    ...
    $Delete->exeDelete(
        DB_COMMENTS_LIKES,
        'WHERE user_id NOT IN(SELECT user_id FROM ' . DB_USERS . ') AND id >= :id',
        'id=1'
    );
    $Delete->exeDelete(
        DB_COMMENTS_LIKES,
        'WHERE comm_id NOT IN(SELECT id FROM ' . DB_COMMENTS . ') AND id >= :id',
        'id=1'
    );
    ...
    switch ($Case) {
        ...
        case 'like':
            $Read->fullRead(
                ...
                sprintf('user=%s&comm=%s', $_SESSION['userLogin']['user_id'], $PostData['id'])
            );
            if (!$Read->getResult()) {
        ...
        case 'aprove':
            ...
            if ($Read->getResult()) {
                ...
                $Read->fullRead(
                    'SELECT id FROM ' . DB_COMMENTS . ' WHERE status > 1 AND alias_id = :id',
                    'id=' . $Alias
                );
                if (!$Read->getResult()) {
        ...
        case 'response':
            ...
            if ($Read->getResult()) {
                ...
                if (empty($PostData['comment'])) {
                    ...
                    $jSON['trigger'] = Check::ajaxErro(
                        sprintf(
                            '<b>Oppsss, </b> você esqueceu de escrever a resposta %s, para responder informe a resposta!',
                            $_SESSION['userLogin']['user_name']
                        ),
                        E_USER_WARNING
                    );
                ...
                require __DIR__ . '/../_tpl/Client.email.php';

                $Read->fullRead(
                    'SELECT user_name, user_lastname, user_email FROM ' . DB_USERS . ' WHERE user_id = :id',
                    'id=' . $Comm['user_id']
                );
                ...
                $BodyMail = "
                    ...
                    <p>Qualquer dúvida que venha a ter, responda nos comentários. E vamos responder o mais breve possível.</p>
                    <p>Muito obrigado por sua interação conosco {$Read->getResult()[0]['user_name']}. Espero ter atendido suas expectativas.</p>
                ...
                $Email->enviarMontando(
                    $Read->getResult()[0]['user_name'] . ', seu comentário foi respondido!',
                    $Mensagem,
                    SITE_NAME,
                    MAIL_USER,
                    sprintf('%s %s', $Read->getResult()[0]['user_name'], $Read->getResult()[0]['user_lastname']),
                ...
                if ($ResponseTo != $Comm['user_id']) {
                    ...
                    <p>Acesse agora mesmo o link abaixo para ver minha resposta, e caso queira você pode continuar comentando...<p>
                    <p><a title='Ver Comentário' href='{$Link}#comment{$Create->getResult()}' target='_blank'>VER/RESPONDER COMENTÁRIO!</a></p>
                    <p>Qualquer dúvida que venha a ter, envie no sua resposta. E vamos responder o mais breve possível.</p>
                    <p>Muito obrigado por sua interação conosco {$Read->getResult()[0]['user_name']}. Espero ter atendido suas expectativas.</p>
                    <p><i>{$_SESSION['userLogin']['user_name']} {$_SESSION['userLogin']['user_lastname']} - " . SITE_NAME . '</i></p>
                    ';
                    $Mensagem = str_replace('#mail_body#', $BodyMail, $MailContent);
                    $Email->enviarMontando(
                        $Read->getResult()[0]['user_name'] . ', existe uma nova resposta em seu comentário!',
                        $Mensagem,
                        SITE_NAME,
                        MAIL_USER,
                        sprintf('%s %s', $Read->getResult()[0]['user_name'], $Read->getResult()[0]['user_lastname']),
                        ...
                        $jSON['content'] .= "<article class='ead_support_response ead_support_response_reply reply' id='{$ResponseReply['id']}'>
                                ...
                                $user_reply['user_thumb']
                                    ...
                                    <header class='ead_support_response_content_header'>
                                        <h1>Resposta de <a target='_blank' href='dashboard.php?wc=teach/students_gerent&id={$user_reply['user_id']}' title='{$user_reply['user_name']} {$user_reply['user_lastname']}'>{$user_reply['user_name']} {$user_reply['user_lastname']}</a> dia " . date(
                                'd/m/Y H\hi',
                ...
                $jSON['trigger'] = Check::ajaxErro(
                    "<b>Sua resposta foi enviada com sucesso!</span>"
                );
            } else {
                $jSON['trigger'] = Check::ajaxErro(
                    sprintf(
                        "<span>Desculpe %s, mas não foi possível recuperar o comentário que deseja responder!</span><p><b>Experimente atualizar a página :/</b></p>",
                        $_SESSION['userLogin']['user_name']
                    ),
                    E_USER_ERROR
                );
        ...
        case 'remove':
            ...
            if ($Read->getResult()) {
                ...
                $Read->fullRead(
                    'SELECT id FROM ' . DB_COMMENTS . ' WHERE status > 1 AND alias_id = :id AND id != :this',
                    sprintf('id=%s&this=%s', $Comment, $PostData['del_id'])
                );
                if (!$Read->getResult()) {
                    $UpdateData = ['status' => 1];
            ...
            $Delete->exeDelete(
                DB_COMMENTS_LIKES,
                'WHERE comm_id = :id OR comm_id IN(SELECT id FROM ' . DB_COMMENTS . ' WHERE alias_id = :id)',
                'id=' . $PostData['del_id']
            );
        ...
        case 'edit_response':
            ...
            if ('comment' == $PostData['type']) {
                $jSON['divcontent'] = [sprintf('#%s .response_chars', $PostData['id']) => $PostData['comment']];
            } else {
                $jSON['divcontent'] = [sprintf('#%s .reply_chars', $PostData['id']) => $PostData['comment']];
            }

            $jSON['forceclick'] = sprintf('#%s .j_comment_action_close', $PostData['id']);

            if ($Update->getResult()) {
                if ('comment' == $PostData['type']) {
                    $jSON['trigger'] = Check::ajaxErro(
                        "<b>O comentário foi alterado com sucesso!</span>"
                    );
                } else {
                    $jSON['trigger'] = Check::ajaxErro(
                        "<b>A resposta foi alterada com sucesso!</span>"
                    );
                }
            } else {
                $jSON['trigger'] = Check::ajaxErro(
                    sprintf(
                        "<span>Desculpe %s, mas não foi possível editar o conteúdo!</span>",
                        $_SESSION['userLogin']['user_name']
                    ),
                    E_USER_ERROR
                );
