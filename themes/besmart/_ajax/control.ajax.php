<?php
    ...
    switch ($Action):
        case 'search':
            ...
            $search = $POST['search'];
            $Read->FullRead("SELECT pdt_title, pdt_code, pdt_name, pdt_cover FROM " . DB_PDT . " WHERE pdt_status = :status AND pdt_title LIKE '%' :search '%'", "status=1&search={$search}");
            if ($Read->getResult()):
                $jSON['search'] .= '<ul>';
                foreach ($Read->getResult() as $PDT):
                    $jSON['search'] .= "<li><a href='" . BASE . "/produto/{$PDT['pdt_name']}#pdt' title='{$PDT['pdt_title']}'><img src='" . BASE . "/uploads/{$PDT['pdt_cover']}' alt='{$PDT['pdt_title']}' title='{$PDT['pdt_title']}'/><p>{$PDT['pdt_title']}<br/><span class='new_price'>Cód.: {$PDT['pdt_code']} </span></p></a></li>";
        ...
        case 'get_pdt':
            ...
            if ($Read->getResult()):
                ...
                if (!empty($_SESSION['userLogin']['user_id'])):
                    $Read->FullRead("SELECT wishlist_id FROM " . DB_PDT_WISHLIST . " WHERE pdt_id = :pdt AND user_id = :user", "pdt={$pdt_id}&user={$_SESSION['userLogin']['user_id']}");
                ...
                if ($pdt_inventory >= 1):
                    ...
                    if ($Read->getResult() || !empty($pdt_parent)):
                        ...
                        if ($Read->getResult()):
                            ...
                            $jSON['pdt'] .= "<p>Selecione um <span>padrão</span></p>";
                    ...
                    $jSON['pdt'] .= "<button id='{$pdt_id}' class='wc_cart_less minus' >-</button><input name='item_amount' type='text' value='1' step='{$pdt_step}' max='{$pdt_inventory}' readonly='readonly'/><button id='{$pdt_id}' class='wc_cart_plus plus' >+</button>";
                    $jSON['pdt'] .= "</div>";

                    $jSON['pdt'] .= "<div class='total_content'>";
                    $jSON['pdt'] .= "<div class='purchase_total wc_target_total'>";
                    $jSON['pdt'] .= "<p>Caixa Master: <span>" . ($pdt_master ? $pdt_master : '1 item') . $pdt_unity . "</span></p>";
                    $jSON['pdt'] .= "<p class='margin-10px-top'>Código: <span>{$pdt_code}</span></p>";
                    $jSON['pdt'] .= "</div>";
                    $jSON['pdt'] .= "<div>";
                    ...
                    $jSON['pdt'] .= "<p>Solicite seu orçamento. <a rel='shadowbox' class='wc_tab' href='" . BASE . "/politicas.html' title='Veja as regras'>Veja as regras.</a> <span>Válido em todo território nacional</span></p>";
                    $jSON['pdt'] .= "</div>";
                    $jSON['pdt'] .= "</div>";
                ...
                if (!empty($pdt_content)):
                    ...
                    if ($pdt_drawing) {
                        $jSON['pdt'] .= "<li><a class='' target='_blank' rel='shadowbox' href='" . BASE . "/uploads/{$pdt_drawing}' title='Especificações Técnicas'>Especificações Técnicas</a></li>";
                    }
