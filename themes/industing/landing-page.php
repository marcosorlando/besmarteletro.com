...
<link href="<?php
echo INCLUDE_PATH; ?>/lp/fixedform.css" rel="stylesheet">
<script type="application/javascript" src="<?php
echo INCLUDE_PATH; ?>/lp/js/lp.js"></script>
...
<style type="text/css">
    ...
    *, ::before, ::after {
    ...
    #fixed-form #conversion header h3 {
        font-size: 23px;
        color: <?php echo empty($form_text_color) ? '#f1f1f1' : $form_text_color; ?>;
        font-weight: 400;
        text-transform: uppercase;
        text-align: center;
    ...
    #logo {
        background-color: <?php echo empty($page_header_color) ? '#FFFFFF' : $page_header_color; ?>;
    }
    ...
    .color-hack, #fixed-form {
        background-color: <?php echo empty($form_bg_color) ? '#333333' : $form_bg_color; ?>;
    }


    #conversion-modal .modal-content section div.actions button.call_button, #fixed-form #conversion .actions .call_button {
        background-color: <?php echo $page_btn_bg_color; ?>;
        color: <?php echo $page_btn_text_color; ?>;
    }
    ...
    #conversion-modal .modal-content section div.actions button.call_button:hover, #fixed-form #conversion .actions .call_button:hover {
        background-color: <?php echo $page_btn_bg_color_hover; ?>;
    ...
    .error {
        ...
        -webkit-box-shadow: inset 0px 0px 2px #d10000, 0px 0px 5px #d10000 !important;
        -moz-box-shadow: inset 0px 0px 2px #d10000, 0px 0px 5px #d10000 !important;
        box-shadow: inset 0px 0px 2px #d10000, 0px 0px 5px #d10000 !important;
    }
...
<div id="logo">
	<div class="container">
		<div class="l-box">
            ...
            echo BASE; ?>">
				<img src="<?php
                echo $page_logo ? BASE . ('/uploads/landingpages/' . $page_logo) : INCLUDE_PATH . '/images/logo-color.png'; ?>"
				     alt="<?php
...
<section class="div_gray" id="header-and-image"
         ...
         echo $page_box_color; ?> url(<?php
         echo BASE . '/uploads/landingpages/'
	...
	<div class="container">
		<div class="pure-u-1">
			<div class="l-box">
				...
				<figure>
                    <?php
                    echo $page_mockup ? '<img src= ' . BASE . sprintf(
                            "/uploads/landingpages/%s alt='%s' title='%s' >",
                            $page_mockup,
                            $page_title,
                            $page_title
                        ) : ''; ?>
...
<section id="fixed-form">
	<div id="conversion">
		...
		<section>
			<form class="j_landing_page form_capitalize" method="post" action="" enctype="multipart/form-data">
				<div class="callback_return"></div>
				<input type="hidden" name="callback" value="Lp"/>
				<input type="hidden" name="callback_action" value="manage"/>
				<input type="hidden" name="callback_title" value="<?php
                echo $page_title; ?>"/>
				<input type="hidden" name="callback_link" value="<?php
                echo $page_name; ?>"/>
				<input type="hidden" name="lead_conversion" value="<?php
                echo $page_title; ?>"/>
				<input type="hidden" name="page_destino" value="<?php
                echo $page_destino; ?>"/>
				<input type="hidden" name="page_mockup" value="<?php
                echo $page_mockup; ?>"/>
				...
				<div class="field">
                    ...
                    foreach ($Read->getResult() as $state) {
                        echo sprintf("<option value='%s'>%s</option>", $state['uf'], $state['uf']);
                    }
                    echo '</select>';
                    ?>
				</div>
				...
				<div class="actions">
					<button type="submit"
					        class="call_button"><?php
                        echo empty($page_btn_text) ? 'Acessar Conteúdo' : $page_btn_text; ?>
						<img class="form_load none" style="margin-left: 10px;" alt="Enviando Requisição!"
						     title="Enviando Requisição!" src="<?php
                        echo INCLUDE_PATH; ?>/lp/ajax-loader.gif"/>
					</button>
				</div>
...
<section id="content" style="background: <?php
echo $page_bg_color; ?>">
...
<script>
    $(function () {
        var BASE = '<?php echo INCLUDE_PATH; ?>';
...
<script type="text/javascript" src="<?= BASE; ?>/_cdn/text.control.js"></script>
