...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] = $CallBack) {
    ...
    if ('manage' === $Case) {
        if (\in_array('', $PostData)) {
            ...
            $jSON['trigger'] = Check::ajaxErro(
                '<strong>OPPSSS:  </strong>'.$PostData['lead_name'].', para receber seu material, realize a soma dos números corretamente',
                E_USER_WARNING
            );
        } else {
            $Create = new Create();
...
<!-- ////// linha necessária para não quebrar o footer no Mac Mail //// -->     
    <tr>
      <td colspan='3' width='100%'></td>
            ...
            $jSON['trigger'] = Check::ajaxErro(
                '<strong>Obrigado!</strong> Um link foi enviado para seu e-mail para salvar seu E-book!'
            );
            $jSON['redirect'] = $Link;
            // endif;
        }
    ...
    if ($jSON) {
        ...
        $jSON['trigger'] = Check::ajaxErro(
            '<strong class="icon-warning">OPSS:</strong> Desculpe. Mas uma ação do sistema não respondeu corretamente. Ao persistir, contate o desenvolvedor!',
            E_USER_ERROR
        );
        echo \json_encode($jSON);
    }
