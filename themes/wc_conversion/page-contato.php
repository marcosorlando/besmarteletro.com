        ...
        <div class="page_content">
            ...
            <div class="htmlchars">
                ...
                if ($Contato && $Contato['action'] == 'contact'):
                    ...
                    if (in_array('', $Contato)):
                        Check::erro("Para enviar seu contato, favor preencha todos os campos!", E_USER_WARNING);
                    elseif (!Check::email($Contato['email']) || !filter_var($Contato['email'], FILTER_VALIDATE_EMAIL)):
                        Check::erro("Desculpe, mas o e-mail que você informou não tem um formato válido!", E_USER_ERROR);
                    else:
                        ...
                        $Email ??= new Email();
                        $Email->enviarMontando($Contato['assunto'], $MailContent, $Contato['nome'],
	                        $Contato['email'], SITE_ADDR_NAME, MAIL_USER);
                        if (!$Email->getError()):
                            $_SESSION['sucesso'] = "Sua mensagem foi enviada com sucesso!";
                            header('Location: ' . BASE . '/contato');
                            exit;
                        else:
                            Check::erro("Desculpe, não foi possível enviar sua mensagem. Entre em contato via " .
	                            SITE_ADDR_EMAIL . ". Obrigado!", E_USER_ERROR);
                        endif;
                    endif;
                endif;

                if (!empty($_SESSION['sucesso']) && empty($Contato)):
                    Check::erro($_SESSION['sucesso']);
